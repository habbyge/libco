 ...
 * 解决方案a：最简单粗暴的扩大栈空间比如512K，这种方法用更大的内存占用来降低爆栈风险。得不偿失，而且只是降低爆栈风险而已，并不能完全解决问题，不推荐。
 * 解决方案b：把常用变量类型和自定义类型都包装一下，改变内存分配方式为在堆内分配，参考：改造局部变量的内存分配方式，这样基本可以解决栈溢出的问题，
 *          但是协程化的时候，代码变动比较多，而且对开发者的约束也比较多，不太方便。
 * 解决方案c：找一个合适的时机，计算一下当前栈使用情况，如果占用空间超过阈值比如80%的时候，申请一块更大的内存代替当前栈空间，并把相关寄存器和栈内容拷
 *          贝过来，销毁旧的栈空间。等协程回收的时候，再将扩大的栈缩小为默认大小。参考 libco栈自动扩容。这种解决方案有个缺陷，就是不能有 指向栈变量
 *          的指针或引用，因为栈拷贝之后，指针指向的数据位置已经变化了，但是指针的值没有变化，是有很大安全隐患的。
 * 解决方案d：利用 malloc 的一个特性，就是 malloc 内存的时候，写数据之前，其实只是从虚拟地址空间里分配了一段地址范围，没有实际分配物理内存，只有实
 *          际写入数据的时候才发生缺页中断，分配需要大小的物理内存。利用这个特性，可以让协程先分配一个比较大的空间，比如8M。当大量的协程创建的时候，
 *          总的分配出来内存可以远大于实际物理内存，64位服务器上最多可以分配 2^48 bit的虚拟地址空间，所以是够用的。实际运行过程中大部分协程实际分
 *          配的物理内存都是在一定阈值之内，比如128K，偶尔有些超出阈值的，只要没有超过8M，就不会有堆栈溢出。对于这些超出阈值分配的空间，每次协程回
 *          收的时候都检测一下是否有超出，如果超出的话，做一个回收就可以保证物理内存在一个可控范围之内。这里会用到两个系统调用：mincore 和 madvise。
 *          因为超出阈值分配的协程肯定是小概率的，所以对服务器性能的影响很小。具体实现方案参考 让协程私有栈足够大。推荐这种实现方案。
 * 以上的所有解决方案都是最大程度防止溢出，并不能完全避免。当有代码突然分配一个超大的栈空间的时候，还是会出现溢出。而且当程序发生栈溢出的时候，如果溢
 * 出部分写入的内存是可写的，往往不会直接触发bug，只有当被写坏的内存数据被使用的时候才会发现bug，但这个时候已经不在事故现场了。为了能及时发现这种bug，
 * 在分配的栈空间的栈顶，对libco来说，就是在分配出来的栈空间的低地址位置，设置一定空间比如 4K 为只读。这样当将要发生溢出，第一次写这块内存的时候就会
 * 触发异常抛出，及时发现bug现场。具体实现方案需要一个系统调用 mprotect，只需要第一次创建协程的时候调用一次，不会影响整个运行效率。
 * 2、数据重入： 协程中如果需要一个全局的数据做计算的话，在协程挂起，再唤醒的时候，这个数据可能已经被其它协程改写，影响当前协程的计算。如果是普通数据
 * 类型，可以用 libco 的协程级私有变量 ROUTINE_VAR 快速改造，避免重入； 但是在游戏中经常有一种场景：假设需要遍历所有玩家，对每个玩家做个异步的操作，
 * 比如落地一个置脏数据，协程化之后，就变成用迭代器迭代一个全局容器，在迭代循环内有协程挂起操作。这样在协程唤醒继续循环的时候，可能容器本身已经发生了变
 * 化，使当前的迭代器指向一个错误的成员导致bug发生。
